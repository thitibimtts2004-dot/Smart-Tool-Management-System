...
- [ ] Spawn Reviewer agent (haiku · read-only · all platforms):
      Prompt template (fill before spawn):
        "You are a read-only reviewer. Do NOT edit any file.
        1. Read .sessions/mece_plan.md → find all lines starting with 'Verify-N:'
        2. Run each Verify-N bash command exactly as written
        3. PASS = exits 0 AND output matches expected · FAIL = any other result
        Report: PASS list (section + criterion) · FAIL list (section + criterion + actual output)
        Reply 'PASS' if all pass · 'FAIL: <section>: <reason>' if any fail"
      On PASS → proceed · On FAIL → fix → retry 1× → R13
      skip only if task was read-only (no src/ writes)
...
- [ ] Write Phase 0 carry-forward → `.sessions/session_handoff.md` (Tool: Write): `skill_name + CFP_COUNT + task + mece_plan_hash=<sha1sum .sessions/mece_plan.md | cut -c1-8> + resume_at=S<N>:step:<desc>` — survives /compact + enables new-chat resume
...
- [ ] Check provider: `grep "^platform:" .agents/platform/detected.md`
      claude-code → /compact — ALWAYS run (carry-forward written FIRST · prevents next-task context bloat)
                    → ✅ แจ้ง user: "compact เรียบร้อยครับ — เปิด chat ใหม่แล้วสั่งงานต่อได้เลยครับ"
      other       → compact_state.md พร้อมแล้ว → แจ้ง user: "Session ปิดแล้วครับ — เปิด chat ใหม่แล้วสั่งงานต่อได้เลย"
...
During Phase 3 execution — check these conditions. If triggered, load the referenced section only.

| Condition | Trigger | Load (offset+limit) |
|---|---|---|
| Sub-agent spawn needed | probe ≥5 files OR section >8 steps | `agent/SKILL.md` §Spawn Rules |
| OmO roles needed | sections > 2 OR any [gate]/DB action | `agent/SKILL.md` §OmO |
| CFP pattern found | R16 self-improve fires | `self_improve/SKILL.md` §CFP Logging |
| DB edit detected | any `src/db/` touch | `INVARIANTS.md` §I2 (grep offset first) |
| Error recurring | "still broken" / same ERR-XXX in roadmap | `error_index.md` ERR-XXX entry |
| Session close | user says "ปิด/close/done" | `session_manager/SKILL.md` §3 |
| Token >60k | SESSION_TOTAL threshold | `session_manager/SKILL.md` §2 TOKEN PAUSE |

Rule: never pre-load these — load only when condition is true. 1 lookup + 1 targeted Read = 2 tool calls max.
