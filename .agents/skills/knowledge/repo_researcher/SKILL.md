...
## hand-off
> Spec: docs/session_templates/handoff_block_schema.md · this skill OWNS the research findings; doc_builder may pull them as EXTRA input (T-202 single-source rule).
downstream: doc_builder
artifact: research summary (the synthesized findings file)
format: knowledge/research/<repo_name>_summary.md
role: supplementary
note: SUPPLEMENTARY — doc_builder's primary source stays the live repo it documents. The research summary is an extra reference, never doc_builder's main input.
required-inputs:
  - knowledge/research/<repo_name>_summary.md exists and `[research-done]` was emitted
on-missing: skip the offer silently — supplementary means doc_builder proceeds from its primary source anyway (NO `[handoff-blocked]`, NO HALT)
