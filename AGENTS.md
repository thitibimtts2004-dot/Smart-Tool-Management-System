...
[C0.5] → each turn before C1: read [token-state] hook (LOOP_W · SESSION · CHAT) · NOTE [token-state] is a start-of-turn / prior-turn-end snapshot (lags reality by up to 1 turn · CFP-041) → at any mid-turn DECISION point (compact_checkpoint · a threshold below) OR on a heavy-tool turn (≥5 tool calls / clone / bulk-copy) grep LIVE `.sessions/session_tokens.md` before deciding — the PostToolUse hook keeps that file current within the turn (main-context turns ONLY — subagents pollute it · CFP-041) · PRIMARY signal = CHAT_TOTAL (real context size) · LOOP_WEIGHT = SECONDARY tool-call-count hint only · CHAT_TOTAL >80k → [compact-rec] strong: surface recommendation block (Recommend/Why/MUST-vs-SHOULD=SHOULD/Resume brief/Your call) — NOT a STOP, user decides · LW >50 → [compact-rec] light hint only (secondary backstop · optional · no STOP) · HARD STOP only at the real ceiling SESSION_TOTAL >90k OR CHAT_TOTAL >120k → [compact-STOP] write compact_state.md → STOP · skip required tier = CFP-026
...
Compact check (every turn): use hook `[token-state]` values, not `session_tokens.md` (subagents overwrite it) — EXCEPT at a mid-turn DECISION point or on a heavy-tool turn (≥5 calls / clone / bulk-copy), where on a main-context turn you MUST grep LIVE `session_tokens.md` because [token-state] is a start-of-turn snapshot and lags by up to 1 turn (CFP-041). Thresholds → see C0.5 (§Per-Turn Routing): PRIMARY = CHAT_TOTAL · LOOP_WEIGHT secondary · hard STOP only at SESSION_TOTAL >90k OR CHAT_TOTAL >120k → [compact-STOP].
