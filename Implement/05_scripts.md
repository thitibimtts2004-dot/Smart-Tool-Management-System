...
| `cfp_fix_probe.py` | **Self-improvement loop stage 7 (Measure-stays-fixed · T-265)** — scans resolved CFPs (with `resolved_by`), asserts each `artifacts[]` path exists + (hook scripts) still wired in settings.json → `[cfp-fix-drift]`/`[cfp-fix-ok]`; fail-open (exit 0 on any error, exit 2 on drift) | boot_init.sh (B1) + Stop hook (automatic) · manual: `python3 scripts/cfp_fix_probe.py` |
| `cfp_recurrence.py` | **Self-improvement loop stage 8 (Re-open · T-265)** — agent-INVOKED reliable write: flips a CFP status resolved→reopened, appends today to `recurred_after_fix[]`, `count++`, prints `[recurrence-logged]`/`[fix-required]`/`[fix-escalated]`. Detection stays R16/BC-E judgment | `python3 scripts/cfp_recurrence.py CFP-N [--date YYYY-MM-DD]` (when an agent JUDGES a resolved CFP recurred — see harness_doctor §1.4) |
